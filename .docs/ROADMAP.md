...
- ⚙️ **Tối ưu lọc khi kho lên chục nghìn bài** — *(**XONG 17/07/2026, NK37** — nghiệm thu GUI OK + merged `--no-ff` master; spec+plan `2026-07-17-toi-uu-loc*`)*. Sửa: tính sẵn bảng tra "nhánh → hệ gốc" (`buildRootHeMap` trong `classification.js`, memo theo cây) **một lần** thay vì `getRootHeId` leo ngược cây cho **mỗi bài × mỗi lần lọc**. Fallback `?? cid` giữ tương đương tuyệt đối với bản cũ (kể cả id mồ côi). Kết quả lọc / giao diện / xuất `.tex` **không đổi** (108 test Vitest xanh, golden byte-identical, build 0 warning; đúng 3 file: `DataGrid.jsx`·`classification.js`·`classification.test.js`). Ở 2000 bài vốn tức thời — thay đổi **vô hình** hôm nay, chỉ có ý nghĩa khi kho thật lớn.
...
- **Tiến độ (17/07/2026):** **Đợt A + Đợt B đã ĐÓNG.** Đang ở **Đợt C**: (1) **Ghi transaction all-or-nothing** — **XONG** (NK35, v1.2.1). (2) **CRA → Vite (+Vitest)** — **XONG** (NK36: 104 test Vitest xanh, golden `.tex` byte-identical, nghiệm thu GUI OK, merge `--no-ff` master, đóng gói **v1.2.2**). **Chốt hướng Đợt C còn lại (17/07):** **BỎ** In/xem PDF + Đồng bộ nhiều máy (Thầy không cần); **chỉ làm nốt Tối ưu lọc** rồi đóng Đợt C. **(3) Tối ưu lọc — XONG (NK37, 17/07: 108 test, golden byte-identical, 0 warning; nghiệm thu GUI OK; merged `--no-ff` master).** ✅ **ĐỢT C ĐÓNG HOÀN TOÀN — cả Đợt A + B + C đều xong.**
