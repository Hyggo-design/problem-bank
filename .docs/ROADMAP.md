...
- **Tiến độ (16/07/2026):** **Đợt A + Đợt B đã ĐÓNG.** Đang ở **Đợt C**: mục đầu **Ghi transaction all-or-nothing** đã **build + kiểm tự động XONG** (NK35, nhánh `feat/ghi-tron-goi-transaction`, 5 commit chưa merge) — còn **nghiệm thu GUI** (`tauri dev`) + **đóng gói `.msi` v1.2** để chốt. Mục Đợt C kế tiếp chưa chốt: (2) **CRA → Vite** (nền tảng build, dev/HMR nhanh hơn) thường là bước hợp lý sau độ-tin-cậy; **In/xem PDF trong app** và **Đồng bộ nhiều máy** nặng + tuỳ nhu cầu; **Tối ưu lọc** chỉ cần khi kho thật sự lớn.
