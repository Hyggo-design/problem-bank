...
- 🧩 **Ma trận v2** — *(đợt con 1 — thêm chiều **Loại câu** vào lưới bốc câu: **XONG 14/07/2026, NK31**, spec+plan `2026-07-14-ma-tran-v2-loai-cau*`)*. ~~Còn: lưu mẫu ma trận đã đặt tên, điểm số mỗi ô/tổng đề.~~ **ĐÃ BỎ (15/07/2026, NK34): Thầy chốt không làm nốt** — ma trận hiện đã đủ dùng. ~~(tuỳ) xuất kèm **bảng đặc tả**~~ — **ĐÃ BỎ (14/07/2026).**
...
**➡️ ĐỢT B ĐÓNG (15/07/2026, NK34)** — mọi mục đã XONG (tag · ma trận loại-câu · quét trùng) hoặc BỎ (xuất đề+đáp án riêng · bảng đặc tả · cỡ chữ · lưu mẫu/điểm số ma trận). Chuyển sang **Đợt C**.

## Đợt C — Đầu tư dài hạn *(tùy chọn · thay đổi lớn · vài tuần+)*

- ⚙️ **Đổi công cụ build CRA → Vite** — *(**XONG 17/07/2026, NK36**: build + kiểm tự động (**104 test Vitest xanh**, golden `.tex` **byte-identical**) + **nghiệm thu GUI OK**, merge `--no-ff` master; spec+plan `2026-07-17-cra-sang-vite*`. **Đóng gói v1.2.2** — `.msi`+`.exe`, nhẹ hơn v1.2.1 ~1 MB.)* `react-scripts` đã cũ; Vite là mặc định của Tauri, dev/HMR nhanh hơn (build **~0.9s** vs CRA ~1–2 phút), bundle nhẹ, khởi động nhanh hơn. Test chuyển `react-scripts test`(Jest) → **Vitest** (12/14 tệp giữ nguyên, chỉ 2 tệp đổi `jest.*`→`vi.*`). **KHÔNG** đụng nghiệp vụ/schema/Rust/xuất `.tex`; chỉ đổi 3 đuôi `.js`→`.jsx` (có JSX) + 1 biến `.env`.
- 🧩 ~~**In / xem PDF ngay trong app**~~ — **ĐÃ BỎ (17/07/2026, NK36): Thầy chốt không cần.** Nặng (cần nhúng engine LaTeX) & đổi hẳn workflow; xuất `.tex` rồi biên dịch ngoài vẫn ổn.
- ⚙️ **Tối ưu lọc khi kho lên chục nghìn bài** — *(**ĐÃ BUILD + kiểm tự động XANH 17/07/2026, NK37**; spec+plan `2026-07-17-toi-uu-loc*`; **chờ Thầy nghiệm thu GUI**)*. Sửa: tính sẵn bảng tra "nhánh → hệ gốc" (`buildRootHeMap` trong `classification.js`, memo theo cây) **một lần** thay vì `getRootHeId` leo ngược cây cho **mỗi bài × mỗi lần lọc**. Fallback `?? cid` giữ tương đương tuyệt đối với bản cũ (kể cả id mồ côi). Kết quả lọc / giao diện / xuất `.tex` **không đổi** (108 test Vitest xanh, golden byte-identical, build 0 warning; đúng 3 file: `DataGrid.jsx`·`classification.js`·`classification.test.js`). Ở 2000 bài vốn tức thời — thay đổi **vô hình** hôm nay, chỉ có ý nghĩa khi kho thật lớn.
- 🧩 ~~**Đồng bộ nhiều máy / kho dùng chung**~~ — **ĐÃ BỎ (17/07/2026, NK36): Thầy chốt không cần** (dùng một máy là đủ).
- ⚙️ **Ghi "trọn gói" (transaction) cho thao tác lưu** — *(**XONG 16/07/2026, NK35**: build + kiểm tự động + **nghiệm thu GUI OK**, merge `--no-ff` master; spec+plan `2026-07-16-ghi-tron-goi-transaction*`. **Đóng gói v1.2.1 XONG** — `.msi` + `.exe` đã build (16/07) & đẩy `origin/master` ⇒ mục này **đóng hoàn toàn**.)* *(bổ sung từ Đợt A #1, 13/07)*. Fix Đợt A #1 đã **phát hiện & báo lỗi** khi lưu, nhưng chưa đảm bảo *all-or-nothing*: `addProblem`/`updateProblem` ghi bảng `problems` xong mới ghi 3 bảng phân loại (nhiều bước tách rời); `saveImportedProblems` ghi theo từng chunk 50 bài. Hỏng giữa chừng → ghi dở dang (bù lại: bấm lưu lại **tự sửa** nhờ `INSERT OR REPLACE` + xoá-rồi-ghi). Vướng kỹ thuật: plugin SQL của Tauri dùng **pool kết nối** nên `BEGIN/COMMIT` thủ công không chắc ăn (2 lệnh có thể rơi vào 2 kết nối khác nhau) → cần gói transaction đúng cách (ghim 1 kết nối, hoặc đưa lệnh transaction xuống Rust). Là bước làm chắc 100% *sau khi* Đợt A #1 đã báo lỗi tin cậy.
...
- **Tiến độ (17/07/2026):** **Đợt A + Đợt B đã ĐÓNG.** Đang ở **Đợt C**: (1) **Ghi transaction all-or-nothing** — **XONG** (NK35, v1.2.1). (2) **CRA → Vite (+Vitest)** — **XONG** (NK36: 104 test Vitest xanh, golden `.tex` byte-identical, nghiệm thu GUI OK, merge `--no-ff` master, đóng gói **v1.2.2**). **Chốt hướng Đợt C còn lại (17/07):** **BỎ** In/xem PDF + Đồng bộ nhiều máy (Thầy không cần); **chỉ làm nốt Tối ưu lọc** rồi đóng Đợt C. **(3) Tối ưu lọc — ĐÃ BUILD + kiểm tự động XANH (NK37, 17/07: 108 test, golden byte-identical, 0 warning); CHỜ Thầy nghiệm thu GUI → sau khi OK + merge, ĐỢT C ĐÓNG HOÀN TOÀN.**
