...
- **Tiến độ (16/07/2026):** **Đợt A + Đợt B đã ĐÓNG.** Đang ở **Đợt C**: mục đầu **Ghi transaction all-or-nothing** đã **XONG** (NK35 — build + kiểm tự động + **nghiệm thu GUI OK**, đã **merge `--no-ff` master**); **đóng gói v1.2.1** (`.msi`+`.exe`) & đẩy `origin/master` ⇒ **mục #1 đóng hoàn toàn**. Mục Đợt C kế tiếp chưa chốt: (2) **CRA → Vite** (nền tảng build, dev/HMR nhanh hơn) thường là bước hợp lý sau độ-tin-cậy; **In/xem PDF trong app** và **Đồng bộ nhiều máy** nặng + tuỳ nhu cầu; **Tối ưu lọc** chỉ cần khi kho thật sự lớn.
